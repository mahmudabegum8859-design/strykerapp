 ...
 */
package com.opxdemon.terminal.ui.pm.view;
...
public class RecyclerTabLayout extends RecyclerView {
  ...
  protected void scrollToTab(int position, float positionOffset, boolean fitIndicator) {
    ...
    if (selectedView != null) {
      ...
      float sLeft = (position == 0) ? 0 : width / 2.f - selectedView.getMeasuredWidth() / 2.f;
      float sRight = sLeft + selectedView.getMeasuredWidth();

      if (nextView != null) {
        float nLeft = width / 2.f - nextView.getMeasuredWidth() / 2.f;
        float distance = sRight - nLeft;
