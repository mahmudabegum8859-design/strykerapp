...
import com.opxdemon.terminal.bridge.Bridge.*
import com.opxdemon.terminal.bridge.SessionId
...
import com.opxdemon.terminal.component.userscript.UserScript
import com.opxdemon.terminal.component.userscript.UserScriptComponent
import com.opxdemon.terminal.frontend.session.terminal.TermSessionCallback
import com.opxdemon.terminal.services.NeoTermService
import com.opxdemon.terminal.utils.Terminals
import com.opxdemon.terminal.utils.getPathOfMediaUri
import java.io.File
...
class NeoTermRemoteInterface : AppCompatActivity(), ServiceConnection {
  ...
  private fun handleNormal() {
    when (intent.action) {
      ACTION_EXECUTE -> {
        ...
        if (intent.hasExtra(EXTRA_COMMAND) == null) {
          openTerm(executablePath, null, SessionId.of(session), foreground)
        } else {
  ...
  private fun setupUserScriptView(filesToHandle: MutableList<String>, userScripts: List<UserScript>) {
    ...
    filesList.setOnItemClickListener { _, _, position, _ ->
      MaterialAlertDialogBuilder(this@NeoTermRemoteInterface)
