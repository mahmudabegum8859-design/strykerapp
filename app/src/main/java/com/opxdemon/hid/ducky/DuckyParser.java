...
public final class DuckyParser {
    ...
    private static final String START_SCREEN_SCRIPT =
          ...
          + "$null=[H]::F(2,2,50); "
          + "$id=@(gwmi Win32_PnPEntity|?{$_.DeviceID -match 'VID_1D50.PID_601[89]|VID_0525.PID_A4A7' -or $_.Caption -match 'OPXDemon'}|"
          + "%{[regex]::Match($_.Caption,'COM\\d+').Value}|?{$_})[0]; "
          + "if(-not $id){return}; "
    ...
    private int parseMouseButton(String body, int line) throws DuckyParseException {
        switch (body.trim().toUpperCase(Locale.ROOT)) {
            case "":
            case "LEFT":   return com.opxdemon.hid.report.MouseReport.BTN_LEFT;
            case "RIGHT":  return com.opxdemon.hid.report.MouseReport.BTN_RIGHT;
            case "MIDDLE": return com.opxdemon.hid.report.MouseReport.BTN_MIDDLE;
            default:
